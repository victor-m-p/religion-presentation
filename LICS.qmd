...
::: {.notes}
- might regress on time to approach causality from observations.
- might perform experiments to get causality. 
:::

## Why does culture look this way?
![](LICS/intro-1.svg){ width="85%" }
...
::: {.notes}
conceptualize as one connected system of forces.
connections are non-directed at parameter level,
i.e. bi-directional constraints. 
::: 

## The Landscape View {.smaller}
::: {.column width="45%"} 
Parameters (*pairwise constraints* and *local fields*) imply a probability distriubtion over *configurations*

*Equilibrium points*: Locally (meta)stable solutions to the landscape. Interesting when multiple exist. Configurations "higher" in the landscape better balance constraints. 
:::
...
::: {.notes}
Based on parameters 
This we do not get directly from parameters. 
(i.e. the standard approach). 
Interesting, e.g.:
* We want to keep a system in a certain equilibrium
* We want to push a system out of a bad equilibrium
You could imagine this being interesting in many contexts: 
e.g., we are looking at NSDUH now
...
- Some clever modifications to infer a lot of (basically) logistic regressions at the same time. 
:::
...
## $\mathcal{Q}_2$: Dynamics & Equilibria
![](cognitive-attractors/svg/local_global_stability.svg){ width=60% }
...
- Maturation of religions; extreme sects, then mainstream (retaining core).
:::
